...
namespace Camera
{
    public class CameraHandler : MonoSingleton<CameraHandler>
    {
        ...
        [Range(0f, 20f)]
        [SerializeField] private float _yDamping = 0f;
        [Range(0f, 20f)]
        [SerializeField] private float _xDamping = 0f;

        [Header("DeadZone")]
        [Range(0f, 1f)]
        [SerializeField] private float _deadZoneHeight = .7f;
        [Range(0f, 1f)]
        [SerializeField] private float _deadZoneWidth = .7f;
        [Range(0f, 1f)]
        [SerializeField] private float _deadZoneDepth = 0f;

        [Header("Zoom Settings")]
        [SerializeField] private float _zoomStep = 50f;
        [Range(30, 500)]
        [SerializeField] private float _zoomCameraDistance = 50f;
        ...
        [SerializeField] private float _zoomInCameraAngleY = 0f;
        [SerializeField] private float _zoomOutCameraAngleY = 0f;
        [SerializeField] private float _zoomInCameraAngleZ = 0f;
        [SerializeField] private float _zoomOutCameraAngleZ = 0f;
        [SerializeField] private float _mobileZoomInCameraAngleX = 25f;
        [SerializeField] private float _mobileZoomOutCameraAngleX = 45f;
        [SerializeField] private float _mobileZoomInCameraAngleY = 0f;
        [SerializeField] private float _mobileZoomOutCameraAngleY = 0f;
        [SerializeField] private float _mobileZoomInCameraAngleZ = 0f;
        [SerializeField] private float _mobileZoomOutCameraAngleZ = 0f;
        [SerializeField] private bool _enableCameraZoomRotation = true;

        [SerializeField] private bool _cameraSmoothness = true;
        [SerializeField] private float _zeroSmoothnes = 0f;
        ...
        private void LateUpdate()
        {
            ...
            else if (!_isPressed && CameraState == State.Click)
            {
                ...
                if (Physics.Raycast(
                    ray: _mainCamera.ScreenPointToRay(DefaultActions.UI.Point.ReadValue<Vector2>()),
                    hitInfo: out var hit,
        ...
        private void OnEnableCameraCameraRotation(bool status) => _enableCameraZoomRotation = status;

        private void OnEnableSmoothness(bool condition) => _cameraSmoothness = condition;
        ...
        private void InteractableSelected(Vector3 pos, bool withDeadZone = true, bool withDamping = true, bool fromScript = false)
        {
        ...
        private IEnumerator ZoomPinchDetection()
        {
            ...
            while (true)
            {
                ...
                var currentScreenDistance = Vector2.Distance(DefaultActions.UI.PrimaryFingerPosition.ReadValue<Vector2>(),
                    DefaultActions.UI.SecondaryFingerPosition.ReadValue<Vector2>());
                if (prevDist == 0)
        ...
        private void RotateCamera(bool positive, bool useMobile = false)
        {
            ...
            if (useMobile)
            {
                ...
                {
                    stepY = ZoomStepPerc * (Mathf.Abs(_mobileZoomInCameraAngleY) - Mathf.Abs(_mobileZoomOutCameraAngleY));
                    targetRotationY = positive
                        ? Mathf.Clamp(_cameraRotation.y - Mathf.Abs(stepY), _mobileZoomOutCameraAngleY, _mobileZoomInCameraAngleY)
                        : Mathf.Clamp(_cameraRotation.y + Mathf.Abs(stepY), _mobileZoomOutCameraAngleY, _mobileZoomInCameraAngleY);
                }
                else
                {
                    stepY = ZoomStepPerc * (Mathf.Abs(_mobileZoomOutCameraAngleY) - Mathf.Abs(_mobileZoomInCameraAngleY));
                    targetRotationY = positive
                        ? Mathf.Clamp(-Mathf.Abs(_cameraRotation.y + Mathf.Abs(stepY)), _mobileZoomInCameraAngleY, _mobileZoomOutCameraAngleY)
                        : Mathf.Clamp(-Mathf.Abs(_cameraRotation.y - Mathf.Abs(stepY)), _mobileZoomInCameraAngleY, _mobileZoomOutCameraAngleY);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    targetRotationY = positive
                        ? Mathf.Clamp(-Mathf.Abs(_cameraRotation.y + Mathf.Abs(stepY)), _zoomInCameraAngleY, _zoomOutCameraAngleY)
                        : Mathf.Clamp(-Mathf.Abs(_cameraRotation.y - Mathf.Abs(stepY)), _zoomInCameraAngleY, _zoomOutCameraAngleY);
                }
...
#endif
            _cameraFollowTarget.position = Vector3.SmoothDamp(_cameraFollowTarget.position, target, ref _velocity, smoothness);
        ...
        private void CameraLockDisable() => _cameraLockXY.enabled = false;
